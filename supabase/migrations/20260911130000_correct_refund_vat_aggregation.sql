...
begin
 ...
 select case when b.total_fils>0 then greatest(0,least(b.tax_fils,round(b.tax_fils::numeric*(total+p_amount_fils)/b.total_fils)::integer)-coalesce((select sum(prior.tax_fils) from public.refunds prior where prior.payment_id=p.id),0)) else 0 end into refund_tax from public.bookings b where b.id=p.booking_id;
...
comment on function public.request_payment_refund(uuid,integer,text,uuid) is '[§8; CLIENT refund/VAT request] Uses an unambiguous prior-refund alias when calculating cumulative VAT credits. Preserves staff authorization, request idempotency, payment locking and the pending/settled refund ceiling.';
