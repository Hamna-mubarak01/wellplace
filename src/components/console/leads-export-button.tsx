...
const EXPORT_PATH = "/api/console/waitlist/export";

const FORWARDED = ["q", "salutation", "sort", "scope", "from", "to", "source", "campaign", "marketing"] as const;

export interface LeadsExportButtonProps {
  className?: string;
  total: number;
}

export function LeadsExportButton({ className, total }: LeadsExportButtonProps) {
    ...
    try {
      ...
      if (!response.ok || !contentType.startsWith(expectedType)) {
        toast.error("The export did not run", {
          description:
            response.status === 403
              ? "Exporting the waitlist needs a Management account."
      ...
      toast.success("Waitlist exported", { description: name });
    } catch {
      toast.error("The export did not run", {
